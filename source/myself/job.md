...
2. 添加事件的方式。addEventListener第三个参数。阻止冒泡。阻止同类型事件执行呢？怎么触发事件？如何区分是用户触发的事件还是js触发的？（通过event.isTrusted判断）

3. 执行函数的方式。是让说call和apply。call和apply的区别。

4. ajax。跨域。window.name具体怎么实现跨域。

5. 做一个项目怎么规划。组件怎么加载进来的？

6. es6用过哪些？var和let的区别。`{a = 1; let a = 2;}`这个语句会报错吗？为什么？let没有变量提升。const呢？可以修改定义的数组和对象的值吗？如果一个异步接口的执行依赖于另一个异步接口的结果，怎么实现？

7. 块级元素，内联元素。

8. 哪些属性会继承？文本相关的属性`text-*, font-*, color`，列表相关的属性`list-*`.

9. line-height设置0.5是怎么计算的？数值乘以元素字体大小。

10. CSS3用过哪些？flex一般用来做怎样的布局？移动端怎么解决布局问题？说媒体查询加em，这样就得有强限制不能设置元素的font-size.实际上应该是媒体查询加rem。

11. margin重叠。

12. vuex中的数值怎么获取。vue数值双向绑定怎么做的？那[数组是怎么监听变化的](https://github.com/youngwind/blog/issues/85)? Object.defineProperty参数，可设置的属性有哪些。
...
11. 转为块级元素的方式。float和position设置绝对定位，这两种方式也可以实现将元素转为块级元素。但是在自己尝试时发现，一些情况（在作为多个行内元素的最后一个元素时）会转为`incline-block`。
...
2. 说一下重构的这个项目是做什么的？消除是怎么实现的？为什么用vue？路由怎么做的？为什么用vuex？放window对象上不是一样吗？你是说vuex做的工作就是和window对象属性一样？不用vuex还可以用什么？

3. 还用过什么框架？vue这类框架为什么能取代jquery？

4. 清除浮动。bfc为什么叫bfc？有什么特性？你是说bfc是为了解决清除浮动而存在的？全称是什么？

5. 用vue和webpack有用什么ES6语法？为什么用let不用var？箭头函数和普通函数的区别？
